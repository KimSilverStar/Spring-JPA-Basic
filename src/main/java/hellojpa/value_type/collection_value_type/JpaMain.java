	...
	public static void main(String[] args) {
		...
		try {
			...
			member.getAddressHistory().add(
					new Address("old1", "street1", "1")
			);
			member.getAddressHistory().add(
					new Address("old2", "street2", "2")
			);
			...
			// 자바 컬렉션의 remove(): 기본적으로 해당 원소를 찾을 때, equals() 메소드 이용
			// => 해당 타입의 equals(), hashCode() 메소드 재정의 필요
			findMember.getAddressHistory().remove(
					new Address("old1", "street1", "1")
			);
			findMember.getAddressHistory().add(
					new Address("newCity", "street", "100")
			);
